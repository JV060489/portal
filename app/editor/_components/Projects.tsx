...
function RenameInput({ node }: { node: NodeRendererProps<TreeNode>["node"] }) {
  ...
  return (
    <Input
      ...
      className="h-5 text-sm bg-neutral-800 border-neutral-600 px-1 py-0 text-white"
...
export default function Projects() {
  ...
  async function handleDelete() {
    ...
    if (isProject) {
      ...
      await fetch(`/api/projects/${projectId}/scenes/${id}`, { method: "DELETE" });
      setProjects((prev) =>
        prev.map((p) =>
          p.id === projectId
          ...
          {loading ? (
            ...
            <Tree<TreeNode>
              ...
              onRename={({ id, name, node }) => {
                // Handle placeholder nodes: create instead of rename
        ...
        <AlertDialogContent className="bg-neutral-950 border-neutral-800">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {deleteTarget?.isProject && " and all its scenes"}. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
function TreeNodeRow({
  ...
  const isPlaceholder = node.id === "__new-project__" || node.id === "__new-scene__";
  ...
  if (isPlaceholder) {
    return (
      <div ref={dragHandle} style={style} className="flex items-center gap-2 px-3 py-1.5 mx-2 my-0.5">
        {isProject ? (
          <svg className="w-3.5 h-3.5 shrink-0 text-neutral-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  return (
    <DropdownMenu open={menuOpen} onOpenChange={setMenuOpen}>
      <DropdownMenuTrigger asChild onContextMenu={(e) => { e.preventDefault(); setMenuOpen(true); }}>
        {rowContent}
      </DropdownMenuTrigger>
      <DropdownMenuContent className="bg-neutral-900 border-neutral-700 min-w-[140px]">
        <DropdownMenuItem
        ...
        <DropdownMenuItem
          className="text-red-400 focus:bg-neutral-800 focus:text-red-300 cursor-pointer"
          onClick={() =>
            setDeleteTarget({
              id: node.id,
              name: node.data.name,
